...
# Coordenadas del aviso: se leen del `center=` del mapa estático de Google que
# trae la página de detalle (`center=-34.63%2C-58.48`). Es lo único presente en
# todas las variantes de HTML que sirve ML: el JSON `"latitude":"..."` aparece
# con curl pero NO en lo que recibe este scraper (verificado en vivo).
_MAP_CENTER_RE = re.compile(r"center=(-?\d+\.\d+)(?:%2C|,)(-?\d+\.\d+)")
...
def fetch_detail(client: httpx.Client, url: str) -> dict[str, Any]:
    ...
    center = _MAP_CENTER_RE.search(html)
    lat = float(center.group(1)) if center else None
    lon = float(center.group(2)) if center else None
